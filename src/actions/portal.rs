    ...
    fn eval(&self, input: i32) -> Result<i32, &'static str> {
        ...
        loop {
            ...
            if let Some(value) = indexed {
                ...
                let to_add: i32 = (value.to_digit(10).ok_or("Can't parse digit char")? as i32)
                    .checked_mul(
                        10i32
                            .checked_pow(out_ as u32)
                            .ok_or("Pow caused overflow")?,
                    )
                    .ok_or("Mul caused overflow")?
                    .checked_mul(sign)
                    .ok_or("Mul caused  overflow")?;

                as_chars.remove(in_);
                output = String::from_iter(as_chars.iter().rev())
                    .parse::<i32>()
...
mod tests {
    ...
    fn portal_positive_1step() {
        let action = PortalAction { in_: 2, out_: 0 };
